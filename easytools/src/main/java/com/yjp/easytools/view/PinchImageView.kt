...
open class PinchImageView : ImageView {
    ...
    override fun setOnClickListener(l: OnClickListener?) { //默认的click会在任何点击情况下都会触发，所以搞成自己的
        mOnClickListener = l
    }

    override fun setOnLongClickListener(l: OnLongClickListener?) { //默认的long click会在任何长按情况下都会触发，所以搞成自己的
     ...
     * @see .getInnerMatrix
     */
    fun getCurrentImageMatrix(matrix: Matrix): Matrix { //获取内部变换矩阵
        var matrix = matrix
    ...
    fun addOuterMatrixChangedListener(listener: OuterMatrixChangedListener?) {
        ...
        } else { //如果监听列表修改被锁定,那么尝试在监听列表副本上添加
...
//使用boolean无法判断递归结束
        ...
        if (mDispatchOuterMatrixChangedLock == 0) { //如果期间有修改列表,那么副本将不为null
            if (mOuterMatrixChangedListenersCopy != null) { //将副本替换掉正式的列表
    ...
    private fun initView() { //强制设置图片scaleType为matrix
    ...
    ////////////////////////////////绘制////////////////////////////////
    override fun onDraw(canvas: Canvas) { //在绘制前设置变换矩阵
        if (isReady()) {
            val matrix = MathUtils.matrixTake()
    ...
    private inner class MaskAnimator(start: RectF, end: RectF, duration: Long) :
        ...
        override fun onAnimationUpdate(animation: ValueAnimator) { //获取动画进度,0-1范围
    ...
    private val mGestureDetector =
        GestureDetector(this@PinchImageView.context, object : SimpleOnGestureListener() {
            ...
            override fun onLongPress(e: MotionEvent) { //触发长按
                if (mOnLongClickListener != null) {
                    mOnLongClickListener!!.onLongClick(this@PinchImageView)
                }
            }

            override fun onDoubleTap(e: MotionEvent): Boolean { //当手指快速第二次按下触发,此时必须是单指模式才允许执行doubleTap
            ...
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean { //触发点击
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        //最后一个点抬起或者取消，结束所有模式
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) { //如果之前是缩放模式,还需要触发一下缩放结束动画
            if (mPinchMode == PINCH_MODE_SCALE) {
                scaleEnd()
            }
            mPinchMode = PINCH_MODE_FREE
        } else if (action == MotionEvent.ACTION_POINTER_UP) { //多个手指情况下抬起一个手指,此时需要是缩放模式才触发
            if (mPinchMode == PINCH_MODE_SCALE) { //抬起的点如果大于2，那么缩放模式还有效，但是有可能初始点变了，重新测量初始点
                if (event.pointerCount > 2) { //如果还没结束缩放模式，但是第一个点抬起了，那么让第二个点和第三个点作为缩放控制点
        ...
        } else if (action == MotionEvent.ACTION_DOWN) { //在矩阵动画过程中不允许启动滚动模式
            if (!(mScaleAnimator != null && mScaleAnimator!!.isRunning)) { //停止所有动画
                cancelAllAnimator()
                //切换到滚动模式
        ...
        } else if (action == MotionEvent.ACTION_POINTER_DOWN) { //停止所有动画
            cancelAllAnimator()
            //切换到缩放模式
            ...
            if (!(mScaleAnimator != null && mScaleAnimator!!.isRunning)) { //在滚动模式下移动
                if (mPinchMode == PINCH_MODE_SCROLL) { //每次移动产生一个差值累积到图片位置上
                    scrollBy(event.x - mLastMovePoint.x, event.y - mLastMovePoint.y)
                    //记录新的移动点
                    mLastMovePoint[event.x] = event.y
                    //在缩放模式下移动
                } else if (mPinchMode == PINCH_MODE_SCALE && event.pointerCount > 1) { //两个缩放点间的距离
    ...
    private fun scrollBy(xDiff: Float, yDiff: Float): Boolean {
        ...
        if (bound.right - bound.left < displayWidth) {
            xDiff = 0f
            //如果图片左边在移动后超出控件左边
        } else if (bound.left + xDiff > 0) { //如果在移动之前是没超出的，计算应该移动的距离
            ...
            //如果图片右边在移动后超出控件右边
        } else if (bound.right + xDiff < displayWidth) { //如果在移动之前是没超出的，计算应该移动的距离
...
//但是后续的scale操作会将xy不等的缩放值纠正,改成和x方向相同
        mScaleBase =
            MathUtils.getMatrixScale(mOuterMatrix)[0] / MathUtils.getDistance(
                x1,
                y1,
                x2,
                y2
...
//FlingAnimator单位为 像素/帧,一秒60帧
    ...
    private inner class FlingAnimator(vectorX: Float, vectorY: Float) :
        ...
        override fun onAnimationUpdate(animation: ValueAnimator) { //移动图像并给出结果
    ...
    private inner class ScaleAnimator @JvmOverloads constructor(
        ...
        override fun onAnimationUpdate(animation: ValueAnimator) { //获取动画进度
            val value = animation.animatedValue as Float
            //根据动画进度计算矩阵中间插值
            for (i in 0..8) {
                mResult[i] = mStart[i] + (mEnd[i] - mStart[i]) * value
    ...
    private abstract class ObjectsPool<T>(
         ...
         * @see .take
         */
        fun given(obj: T?) { //如果对象池还有空位子就归还对象
    ...
    private class MatrixPool(size: Int) :
        ...
        protected override fun resetInstance(obj: Matrix): Matrix {
            obj.reset()
            return obj
        }
    }
    ...
    private class RectFPool(size: Int) : ObjectsPool<RectF>(size) {
        ...
        protected override fun resetInstance(obj: RectF): RectF {
            obj.setEmpty()
            return obj
        }
    }
